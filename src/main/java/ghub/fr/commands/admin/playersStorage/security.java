...
public class security implements CommandExecutor, TabCompleter {
                ...
                if (args.length == 4 || args.length == 3) {
                    if (getDataStorage.playerExistForBonus(sender, Bukkit.getOfflinePlayer(args[0]))) {
                        if (args[1].equalsIgnoreCase("add") && args.length == 4) {
                            ghub.fr.system.security.addSecurity(Bukkit.getOfflinePlayer(args[0]),
                                    ghub.fr.system.security.SecurityList.valueOf(args[2]), Integer.valueOf(args[3]));
                            return true;
                        } else if (args[1].equalsIgnoreCase("remove") && args.length == 3) {
                            ghub.fr.system.security.removeSecurity(Bukkit.getOfflinePlayer(args[0]),
                                    ghub.fr.system.security.SecurityList.valueOf(args[2]));
                            return true;
                        }
                    }
                }
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        ...
        if (args.length == 1) {
            ...
            for (ghub.fr.system.security.SecurityList security : ghub.fr.system.security.SecurityList.values()) {
