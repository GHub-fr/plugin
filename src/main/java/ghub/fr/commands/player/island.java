...
public class island implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (sender instanceof Player) {
            try {
                if (args.length == 0) {
                    if (ghub.fr.menu.island.island.GetHasIsland((OfflinePlayer) sender)) {
                        ghub.fr.menu.island.island.TeleportToIsland((Player) sender);
                    } else {
                        ghub.fr.menu.island.island.CreateIsland((Player) sender);
                    }
                    return true;
                    ...
                    if (Kick().contains(args[0].toLowerCase())) {
                        ...
                        ghub.fr.menu.island.island.SetIslandLocation((Player) sender);
                        return true;
                    ...
                    if (Inviter().contains(args[0].toLowerCase())) {
                        if (getDataStorage.playerExist(Bukkit.getOfflinePlayer(args[1]))) {
                            if (!ghub.fr.menu.island.island.GetHasIsland(Bukkit.getOfflinePlayer(args[1]))) {
                                ghub.fr.menu.island.island.SetPendingIsland(Bukkit.getOfflinePlayer(args[1]),
